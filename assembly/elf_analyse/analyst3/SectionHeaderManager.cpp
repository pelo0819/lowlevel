...
void SectionHeaderManager::RegistSectionIdxName(int i, char *name)
{
    SectionIndexName *in = new SectionIndexName(i, name);
    in->Setup(i, name);
    listIdxName.push_back(in);
}

std::string SectionHeaderManager::get_section_name(int idx)
{
    for(auto &in : listIdxName)
    {
        if(idx == in->getIndex())
        {
            return in->getName(idx);
        }

...
extern int jump(u_int64_t e, int argc, char** argv);
...
int main(int argc, char *argv[])
{
    ...
    char main_addr_str[128];
    FILE *fp;
    fp = fopen("main_addr.txt", "r");
    if(fp != NULL)
    {
        fgets(main_addr_str, 256, fp);
    }

    u_int64_t addr_main = strtol(main_addr_str, NULL, 16);
    printf("[*] addr main() = %ld, 0x%s\n", addr_main, main_addr_str);
    ...
    // asm volatile ("push $0");
    // asm volatile ("push $0");
    // asm volatile ("push %0" :: "m"(file_name));
    // asm volatile ("push $1");
#else
    argv[0] = (char *)(argc -1);
    stackp = &argv[0];
    ...
    printf("=========================\n");
    int d = jump(addr_main, argc - 1, argv2);
    printf("=========================\n");
    printf("[*] ret = %d\n", d);
